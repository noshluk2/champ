...
def generate_launch_description():
    ...
    # Dynamically resolve package paths
    ...
    declare_ros_control_file = DeclareLaunchArgument(
        "ros_control_file", default_value=ros_control_config, description="ROS control configuration path"
    )
    declare_gazebo_world = DeclareLaunchArgument(
        "world", default_value=default_world_path, description="Gazebo world file"
    )
    declare_gui = DeclareLaunchArgument("gui", default_value="true", description="Use Gazebo GUI")
    ...
    bringup_ld = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(config_pkg_share, "launch", "bringup.launch.py")
        ),
        launch_arguments={
    ...
    # Ensure TF synchronization with simulation time
    robot_state_publisher = Node(
        package="robot_state_publisher",
        executable="robot_state_publisher",
        name="robot_state_publisher",
        parameters=[
            {"use_sim_time": True, "frame_id": base_frame}
        ],
        output="screen",
    )
    ...
    return LaunchDescription(
        [
            ...
            bringup_ld,
            gazebo_ld,
            robot_state_publisher,  # Added node for TF synchronization
        ]
    )
